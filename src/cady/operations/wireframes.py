"""Wireframe splitting, pruning, and triangulation algorithms."""
...
def wireframe_to_mesh(wireframe: Wireframe3, *, tolerance: float) -> Mesh3:
    """Convert crossing-prone wire edges into a triangulated mesh."""
    return _mesh_from_triangulation(_triangulate_wireframe(wireframe, tolerance=tolerance))


def remove_dangling_edges(wireframe: Wireframe3) -> Wireframe3:
    """Return a wireframe with recursively dangling edge branches removed."""
    return _wireframe_from_edges(
        wireframe.vertices,
        prune_dangling_edges(wireframe.edges),
    )


def split_crossing_edges(wireframe: Wireframe3, *, tolerance: float = 1e-6) -> Wireframe3:
    """Return a wireframe with edges split at segment crossings."""
    _validate_tolerance(tolerance)
    if not wireframe.edges:
        return _wireframe_cls().from_edges(wireframe.vertices, wireframe.edges)

    vertices = list(wireframe.vertices)
    split_points: list[list[tuple[float, int]]] = [
        [(0.0, edge[0]), (1.0, edge[1])] for edge in wireframe.edges
    ]
    edge_bounds = tuple(_edge_bounds(wireframe.vertices, edge) for edge in wireframe.edges)

    for left_index, left_edge in enumerate(wireframe.edges):
        for right_index in range(left_index + 1, len(wireframe.edges)):
            right_edge = wireframe.edges[right_index]
    ...
    return _wireframe_cls().from_edges(tuple(vertices), tuple(edges))


def triangulate(wireframe: Wireframe3, *, tolerance: float = 1e-3) -> Wireframe3:
    """Split crossings, remove dangling branches, and triangulate loops."""
    return _wireframe_from_triangulation(_triangulate_wireframe(wireframe, tolerance=tolerance))


def triangulate_loops(wireframe: Wireframe3, *, tolerance: float = 1e-3) -> Wireframe3:
    """Detect closed edge cycles and triangulate each into a Wireframe3."""
    return _wireframe_from_triangulation(
        _triangulate_loops_to_triangulation(wireframe, tolerance=tolerance)
    )
...
def _triangulate_wireframe(
    ...
    *,
    ...
    split = split_crossing_edges(wireframe, tolerance=tolerance)
    return _triangulate_loops_to_triangulation(
        remove_dangling_edges(split),
        tolerance=tolerance,
    )


def _wireframe_cls() -> type[Wireframe3]:
    from cady.geometry.wireframe import Wireframe3
...
def _wireframe_from_edges(
    ...
    if not edges:
        return _wireframe_cls().from_edges((), ())
    ...
    return _wireframe_cls().from_edges(compact_vertices, compact_edges)
...
def _triangulation_without_dangling_source_edges(
    ...
    live_edges = prune_dangling_edges(edges)

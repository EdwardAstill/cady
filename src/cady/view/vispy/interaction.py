...
def model_matrix(orbit_center: np.ndarray, orientation: np.ndarray) -> np.ndarray:
    return (translation_matrix(-orbit_center) @ orientation).astype(np.float32, copy=False)
...
class ViewerInteractionState:
    ...
    def from_camera(
        ...
        *,
        ...
        return cls(
            camera=camera,
            orbit_center=np.asarray(camera.target, dtype=np.float32),
            radius=radius,
    ...
    def model_matrix(self) -> np.ndarray:
        return model_matrix(self.orbit_center, self.orientation)

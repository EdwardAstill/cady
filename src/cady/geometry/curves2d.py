...
class ClosedPolyline2D:
    ...
    def to_mesh(self, *, tolerance: float) -> Mesh2D:
        tolerance = positive_tolerance(tolerance)
        from cady.geometry.mesh2d import Mesh2D
        from cady.operations.polygons2d import triangulate_polygon

        point_to_index = {point.tuple(): index for index, point in enumerate(self.vertices)}
        triangles = triangulate_polygon(_point_tuples(self.vertices), tolerance=tolerance)
        faces = tuple(
            (
                point_to_index[a],
                point_to_index[b],
        ...
        return Mesh2D(self.vertices, faces, loop_edges(len(self.vertices)))

...
from cady.operations.polygons2d import dedupe_closed, triangulate_polygon
...
def box_mesh(frame: Frame3D, *, width: float, depth: float, height: float) -> Mesh3D:
...
def cylinder_mesh(
    ...
    *,
    ...
    return Mesh3D(vertices, tuple(faces))


def sphere_mesh(frame: Frame3D, *, radius: float, tolerance: float) -> Mesh3D:
    radius = validate_positive(radius, "radius")
    tolerance = validate_tolerance(tolerance)
    triangles = tuple(
...
def face_mesh(profile: object, frame: Frame3D, *, tolerance: float) -> Mesh3D:
...
def extrusion_mesh(
    ...
    *,
    ...
    end_frame = Frame3D(end_origin, frame.x_axis, frame.normal)
...
def mesh_from_triangles(triangles: tuple[Triangle3, ...]) -> Mesh3D:
    vertices: list[Vec3] = []
    faces: list[tuple[int, int, int]] = []

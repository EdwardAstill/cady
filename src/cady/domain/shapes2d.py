...
def _closed_shape_to_polygon(shape: Shape2D, tolerance: float) -> object:
    ...
    outer = flattened.points()
    holes = tuple(
        curves_to_polyline(hole, tolerance=tolerance).points() for hole in shape.inner_loops
    )
    return ArrayPolygon2(
        as_points2(_point_tuples(outer), name="outer"),
        holes=tuple(as_points2(_point_tuples(hole), name="hole") for hole in holes),
    )
...
class Line(Shape2D):
    ...
    def to_array(self, *, tolerance: float = 1e-3) -> object:
        ...
        return ArrayPolyline2(as_points2(_point_tuples((self.a, self.b))), closed=False)
...
class Arc(Shape2D):
    ...
    def to_array(self, *, tolerance: float = 1e-3) -> object:
        ...
        polyline = curves_to_polyline(self, tolerance=tolerance)
        return ArrayPolyline2(as_points2(_point_tuples(polyline.points())), closed=False)
...
class Circle(Shape2D):
    ...
    def to_array(self, *, tolerance: float = 1e-3) -> object:
        return _closed_shape_to_polygon(self, tolerance)
...
class Polyline(Shape2D):
    ...
    def to_array(self, *, tolerance: float = 1e-3) -> object:
        ...
        return ArrayPolyline2(as_points2(_point_tuples(self.points())), closed=False)
...
class Spline(Shape2D):
    ...
    def to_array(self, *, tolerance: float = 1e-3) -> object:
        if self.closed and self.inner_loops:
            return _closed_shape_to_polygon(self, tolerance)
        from cady.numeric import ArrayBezierSpline2
        from cady.numeric.validation import as_points2

        return ArrayBezierSpline2(
            as_points2(_point_tuples(self.control_points)),
            closed=self.closed,
        )

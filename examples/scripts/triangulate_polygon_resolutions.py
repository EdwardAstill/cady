...
def triangulate_polygon(
    ...
    *,
    ...
    if max_edge_length == "auto":
        return triangulate_polygon_heuristic(
    ...
    return Mesh3(mesh.vertices, mesh.faces, _mesh_edges_from_faces(mesh.faces))
...
def triangulate_polygon_heuristic(
    ...
    *,
    ...
    if len(polygon.faces) != 1:
        raise ValueError("heuristic example expects one polygon face")

    face = polygon.faces[0]
    vertices = tuple(polygon.vertices[index] for index in face)
    nodes, edges, faces = triangulate_mesh3(
        vertices,
        loop_edges(len(vertices)),
        tolerance=tolerance,
        guide=_automatic_guide(vertices, tolerance=tolerance),
    )
    mesh_vertices = tuple((float(x), float(y), float(z)) for x, y, z in nodes)
    mesh_faces = tuple((int(a), int(b), int(c)) for a, b, c in faces)
    mesh_edges = tuple((int(a), int(b)) for a, b in edges)
    return Mesh3(mesh_vertices, mesh_faces, mesh_edges)
...
def _automatic_guide(
    ...
    *,
    ...
    return TriangulationGuide(max_edge_length=max(lengths[-1], span))
...
def _mesh_edges_from_faces(faces: Iterable[tuple[int, ...]]) -> tuple[EdgeIndex, ...]:

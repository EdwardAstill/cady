    ...
    *,
    ...
    while top_index < len(top_chain) - 1 or bottom_index < len(bottom_chain) - 1:
        if top_index == len(top_chain) - 1:
            ...
            bottom_index += 1
        elif bottom_index == len(bottom_chain) - 1:
            face = (
                top_chain[top_index],
                bottom_chain[bottom_index],
            ...
            top_index += 1
        elif top_parameters[top_index + 1] <= bottom_parameters[bottom_index + 1]:
            face = (
                top_chain[top_index],
                bottom_chain[bottom_index],
                top_chain[top_index + 1],
...
def print_scene_summary(
    ...
    print(f"base intersection nodes: {len(unique_points(strip_mesh.base_node_array, tolerance=INTERSECTION_TOLERANCE))}")

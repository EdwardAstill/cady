...
# Set this to a positive number to automatically add display-mesh nodes along
# each longitudinal polyline whenever the original slice-to-slice polyline
# distance is longer than this value. Set it to None to disable this refinement.
MAX_SEGMENT_LENGTH: float | None = 1.0

# None means refine every longitudinal polyline row. Use a tuple such as (1,)
# to refine only the second longitudinal polyline. These are zero-based rows.
...
@dataclass(frozen=True)
class MeshEdge:
    """An undirected edge in a Mesh3D."""
...
def main() -> None:
    ...
    if MAX_SEGMENT_LENGTH is not None:
        node_mesh, node_array = add_distance_limited_nodes_to_linesplan(
            node_mesh,
            node_array,
            linesplan,
    ...
    print_scene_summary(linesplan, wireframe, planes, node_array, node_mesh)
...
def quarter_sphere_linesplan(*, radius: float, samples: int) -> list[Polyline3D]:
    if radius <= 0.0:
...
def node_array_to_point_cloud(
    ...
    *,
    ...
    _require_positive_tolerance(tolerance)
    return PointCloud3D(_unique_points(node_array, tolerance=tolerance))
...
def _row_major_vertices(rows: NodeArray) -> tuple[list[Vec3], list[list[int]]]:
    vertices: list[Vec3] = []
...
def add_node(
    ...
    *,
    ...
    the inserted local node belongs to the unstructured Mesh3D.
    """
    _require_positive_tolerance(tolerance)
    ...
    repaired_mesh, _new_index = add_node_to_current_mesh_edge(
        mesh,
        mesh_edge.start,
        mesh_edge.end,
        new_point,
...
def add_node_to_current_mesh_edge(
    mesh: Mesh3D,
    start_index: int,
    end_index: int,
    point: PointLike3,
    *,
    ...
    """Split an actual current mesh edge and return the inserted vertex index.

    Unlike add_node(), this function is not tied to the original rectangular
    node_array. It can be called repeatedly like this:

        A--B      split at p  ->  A--p--B
        p--B      split at q  ->  A--p--q--B

    That is the operation used by the distance-limited refinement function.
    """
    _require_positive_tolerance(tolerance)

    edge = MeshEdge(start_index, end_index)
    new_point = _as_vec3(point)

    if _points_close(new_point, _mesh_vertex(mesh, start_index), tolerance=tolerance):
        return mesh, start_index
    if _points_close(new_point, _mesh_vertex(mesh, end_index), tolerance=tolerance):
        return mesh, end_index

    new_index = len(mesh.vertices)
    return _split_mesh_edge(mesh, edge, new_point), new_index


def add_distance_limited_nodes_to_linesplan(
    mesh: Mesh3D,
    node_array: NodeArray,
    polylines: Iterable[Polyline3D],
    *,
    ...
    """Apply distance-limited refinement to selected longitudinal rows.

    row_indices=None means every row in node_array. The returned node_array is
    still the original rectangular base grid; all inserted refinement nodes are
    added to the returned Mesh3D so they show up in the visualization.
    """
    _require_positive_tolerance(tolerance)
    _require_positive_length(max_segment_length, name="max_segment_length")

    rows = _as_node_array(node_array)
    _require_rectangular_node_array(rows, tolerance=tolerance)

    selected_rows = tuple(range(len(rows))) if row_indices is None else tuple(row_indices)
    for row_index in selected_rows:
        mesh, rows = add_distance_limited_nodes_to_polyline(
            mesh,
            rows,
            polylines,
            row_index=row_index,
...
def add_distance_limited_nodes_to_polyline(
    mesh: Mesh3D,
    node_array: NodeArray,
    polylines: Iterable[Polyline3D],
    *,
    ...
    """Add local nodes to one source polyline row where spacing is too large.

    The base node_array is assumed to come from split_wireframe_with_planes().
    For the selected row, each neighboring pair of plane-generated nodes is
    measured along the matching source polyline. If that polyline section is
    longer than max_segment_length, extra nodes are inserted at equal arc-length
    intervals so every new section is <= max_segment_length.

    Example:
        section length = 12, max_segment_length = 5
        segment_count = ceil(12 / 5) = 3
        inserted nodes are at arc lengths 4 and 8.

    The returned node_array is unchanged. It remains the structured base grid;
    the inserted local nodes belong to the unstructured Mesh3D.
    """
    _require_positive_tolerance(tolerance)
    _require_positive_length(max_segment_length, name="max_segment_length")
    ...
    rows = _as_node_array(node_array)
    _require_rectangular_node_array(rows, tolerance=tolerance)
    _require_valid_row_index(rows, row_index)

    source_lines = tuple(polylines)
    if row_index >= len(source_lines):
        raise ValueError("row_index selects a node_array row without a source polyline")

    row = rows[row_index]
    source_polyline = source_lines[row_index]

    for left_column in range(len(row) - 1):
        extra_points = _spacing_points_for_row_edge(
            source_polyline,
            row[left_column],
            row[left_column + 1],
            max_segment_length=max_segment_length,
        ...
        current_start = _row_major_index(rows, row_index, left_column)
        current_end = _row_major_index(rows, row_index, left_column + 1)

        # Repeatedly split the current remaining edge:
        #     A--B -> A--p--B -> A--p--q--B -> ...
        for point in extra_points:
            mesh, current_start = add_node_to_current_mesh_edge(
                mesh,
                current_start,
                current_end,
                point,
...
def _spacing_points_for_row_edge(
    polyline: Polyline3D,
    start: Vec3,
    end: Vec3,
    *,
    ...
    distances = _polyline_cumulative_distances(vertices)

    start_distance = _polyline_distance_at_point(
        vertices,
        distances,
        start,
        tolerance=tolerance,
    )
    end_distance = _polyline_distance_at_point(
        vertices,
        distances,
        end,
        tolerance=tolerance,
    ...
    if length <= max_segment_length + tolerance:
        return ()

    segment_count = ceil(length / max_segment_length)
    spacing = length / segment_count
    direction = 1.0 if end_distance >= start_distance else -1.0

    return tuple(
        _point_at_polyline_distance(
            vertices,
            distances,
            start_distance + direction * spacing * index,
        )
        for index in range(1, segment_count)
    )


def _polyline_vertices(polyline: Polyline3D) -> tuple[Vec3, ...]:
...
def _polyline_cumulative_distances(vertices: tuple[Vec3, ...]) -> tuple[float, ...]:
    distances = [0.0]
    for start, end in zip(vertices, vertices[1:], strict=False):
        distances.append(distances[-1] + _distance(start, end))
    return tuple(distances)


def _polyline_distance_at_point(
    vertices: tuple[Vec3, ...],
    ...
    *,
    ...
    for index, (start, end) in enumerate(zip(vertices, vertices[1:], strict=False)):
        ...
        t = max(0.0, min(1.0, t))
        closest = _interpolate_vec3(start, end, t)
        distance_to_segment = _distance(closest, point)

        if distance_to_segment < best_distance:
            best_distance = distance_to_segment
            best_path_distance = distances[index] + t * segment_length

    if best_path_distance is None or best_distance > tolerance:
        raise ValueError("plane-generated node does not lie on the source polyline")
...
def _point_at_polyline_distance(
    ...
    target_distance = max(distances[0], min(distances[-1], target_distance))
    ...
    for index, (start, end) in enumerate(zip(vertices, vertices[1:], strict=False)):
        ...
        return _interpolate_vec3(start, end, t)
...
def _mesh_vertex(mesh: Mesh3D, index: int) -> Vec3:
...
def _split_mesh_edge(mesh: Mesh3D, edge: MeshEdge, new_point: Vec3) -> Mesh3D:
    ...
    return Mesh3D(vertices, tuple(faces), tuple(edges))
...
def _add_nodes_to_scene(
    scene: Scene,
    nodes: Mesh3D | WireframeArray | Iterable[Iterable[Vec3]] | PointCloud3D | None,
) -> Scene:
    if nodes is None:
        return scene
...
def _nodes_to_scene_geometry(
    nodes: Mesh3D | WireframeArray | Iterable[Iterable[Vec3]] | PointCloud3D,
...
def _interpolate_vec3(start: Vec3, end: Vec3, t: float) -> Vec3:
    return Vec3(
...
def _point_segment_parameter(start: Vec3, end: Vec3, point: Vec3) -> float:
...
def _points_close(left: Vec3, right: Vec3, *, tolerance: float) -> bool:
    return (
...
def _require_positive_length(value: float, *, name: str) -> None:
    if value <= 0.0:
        raise ValueError(f"{name} must be positive")


def _require_valid_row_index(rows: NodeArray, row_index: int) -> None:
...
def print_scene_summary(
    ...
    print(f"base intersection nodes: {len(node_array_to_point_cloud(node_array).vertices)}")
    print(
        "display mesh: "
        f"{len(node_mesh.vertices)} vertices, "
        f"{len(node_mesh.edges)} edges, "
        f"{len(node_mesh.faces)} faces"
    )

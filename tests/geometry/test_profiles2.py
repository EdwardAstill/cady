...
def test_profile_circle_wraps_circle_boundary() -> None:
    ...
    assert isinstance(profile.outer, Circle2)
    assert profile.bounds() == (Vec2(1, 2), Vec2(5, 6))
    assert len(profile.to_array(tolerance=0.05).outer) >= 12


def test_profile2_carries_holes_into_polygon_array() -> None:
    profile = Profile2(
        ClosedPolyline2(((0, 0), (5, 0), (5, 5), (0, 5))),
        holes=(ClosedPolyline2(((1, 1), (2, 1), (2, 2), (1, 2))),),
    )

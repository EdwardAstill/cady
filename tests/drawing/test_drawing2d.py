...
from cady.drawing import BlockDefinition, Drawing2D, DrawingEntity, Hatch2D, Layer, Text2D
...
def test_drawing_values_are_frozen_and_mapping_fields_are_read_only() -> None:
    drawing = Drawing2D("front").with_metadata("author", "ed")
...
def test_add_layer_rejects_conflicting_duplicate() -> None:
    drawing = Drawing2D().add_layer(Layer("GEOM", color=3))
...
def test_text_hatch_block_and_insert_entities() -> None:
    ...
    assert isinstance(drawing.entities[0], Text2D)
    assert isinstance(drawing.entities[1], Hatch2D)
    assert drawing.block("MARK") == block


def test_insert_requires_known_block() -> None:
    with pytest.raises(ValueError, match="unknown block"):
        Drawing2D().insert("MISSING", at=(0, 0))


def test_hatch_requires_closed_boundary() -> None:
    with pytest.raises(ValueError, match="closed"):
        Drawing2D().hatch(Curve((0, 0), (1, 0), closed=False))
...
def test_to_arrays_returns_geometry_and_hatch_arrays() -> None:
    drawing = Drawing2D().add(Curve((0, 0), (1, 1))).hatch(Curve((2, 2), (3, 3)))

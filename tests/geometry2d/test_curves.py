...
def test_arc2d_samples_with_tolerance() -> None:
    arc = Arc2D((0, 0), 2, 0, pi / 2)
...
def test_polyline2d_and_closed_polyline2d_array_types() -> None:
    open_polyline = Polyline2D(((0, 0), (1, 0), (1, 1)))
    closed_polyline = ClosedPolyline2D(((0, 0), (1, 0), (1, 1), (0, 0)))
...
def test_circle2d_and_ellipse2d_are_closed_polygon_arrays() -> None:
    ...
    ellipse = Ellipse2D((0, 0), 4, 2, rotation_rad=pi / 6)
...
def test_spline2d_samples_cubic_bezier_to_polyline() -> None:
    spline = Spline2D(((0, 0), (1, 2), (2, 2), (3, 0)))
...
@pytest.mark.parametrize(
    "curve",
    [
        lambda: Line2D((0, 0), (0, 0)),
        lambda: Arc2D((0, 0), 0, 0, 1),
        lambda: Polyline2D(((0, 0),)),
        lambda: ClosedPolyline2D(((0, 0), (1, 0))),
        lambda: Circle2D((0, 0), -1),
        lambda: Ellipse2D((0, 0), 1, 0),
        lambda: Spline2D(((0, 0), (1, 1), (2, 0))),
    ],
)
